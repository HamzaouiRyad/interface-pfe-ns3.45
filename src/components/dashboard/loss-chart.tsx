    ...
    <ChartCard title="Packet Loss (%)">
      <ResponsiveContainer width="100%" height={180}>
        <AreaChart data={data} margin={{ top: 4, right: 12, left: -10, bottom: 0 }}>
          <defs>
            {activeNodes.map(id => (
              <linearGradient key={id} id={`lg${id}`} x1="0" y1="0" x2="0" y2="1">
                <stop offset="5%"  stopColor={NODE_COLORS[id] ?? '#94a3b8'} stopOpacity={0.2} />
                <stop offset="95%" stopColor={NODE_COLORS[id] ?? '#94a3b8'} stopOpacity={0}   />
              </linearGradient>
            ))}
          </defs>
          ...
          <ReferenceLine y={5} stroke="#f59e0b" strokeDasharray="4 4" strokeWidth={1}
            label={{ value: '5%', fill: '#f59e0b', fontSize: 8, fontFamily: 'monospace', position: 'insideTopRight' }} />
          <Tooltip
